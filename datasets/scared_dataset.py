...
class SCAREDDataset(MonoDataset):
    ...
    def get_color(self, folder, frame_index, side, do_flip):
        ...
        image_r_path = self.get_image_path(folder, frame_index, side).replace("/data/", "/data_r/")
        color_r = self.loader(image_r_path)
        if do_flip:
            color = color.transpose(pil.FLIP_LEFT_RIGHT)
            color_r = color_r.transpose(pil.FLIP_LEFT_RIGHT)
