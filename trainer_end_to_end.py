...
import models.swin_transformer.dcmnet as dcmnet
import models.swin_transformer.ht_encoder as h_transformer
import models.afsfm_origin.depth_decoder as depth_decoder
import models.afsfm_origin.resnet_encoder as resnet_encoder
import numpy as np
...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder"] = h_transformer.H_Transformer(window_size=4, embed_dim=64, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32))
        self.models["depth"] = dcmnet.DCMNet(in_channels=[64, 128, 256, 512], in_index=[0, 1, 2, 3], pool_scales=(1, 2, 3, 6),
                                                channels=128,
                                                dropout_ratio=0.1,
                                                num_classes=1,
                                                norm_cfg=norm_cfg,
        ...
        ckpt = torch.load('./checkpoints/best/104checkpoint.pth', map_location='cpu')
    ...
    def predict_poses(self, inputs, disps):
        ...
        if self.num_pose_frames == 2:
            ...
            pose_feats = {f_i: inputs["color_r", f_i, 0] for f_i in self.opt.frame_ids}
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    inputs_all = [pose_feats[f_i], pose_feats[0]]
                    inputs_all_reverse = [pose_feats[0], pose_feats[f_i]]
                    ...
                    outputs[("axisangle_r", 0, f_i)] = axisangle
                    outputs[("translation_r", 0, f_i)] = translation
                    outputs[("cam_T_cam_r", 0, f_i)] = transformation_from_parameters(
                        axisangle[:, 0], translation[:, 0])
                    del outputs[("axisangle_r", 0, f_i)], axisangle, outputs[("translation_r", 0, f_i)], translation, pose_inputs       
        return outputs
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            loss += 0.01 * loss_sift_0 / 2.0
            loss += 0.01 * loss_spatial_0 / 2.0
            total_loss += loss
            losses["loss/{}".format(scale)] = loss
