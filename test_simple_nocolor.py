...
import models.swin_transformer.ht_encoder as ht_encoder
...
def test_simple(args):
    ...
    if args.model_type == 'endodac':
        ...
        encoder = ht_encoder.H_Transformer(window_size=4, embed_dim=64, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32))
        norm_cfg = dict(type='BN', requires_grad=True)
        depth_decoder = dcmnet.DCMNet(in_channels=[64, 128, 256, 512], in_index=[0, 1, 2, 3], pool_scales=(1, 2, 3, 6),
                                                channels=128,
                                                dropout_ratio=0.1,
                                                num_classes=1,
                                                norm_cfg=norm_cfg,
    ...
    if os.path.isfile(args.image_path):
        ...
        # paths_ours = []
        # for path in paths:     
        #     path = path.replace("test", "test_ours")
        #     paths_ours.append(path)
        # print(paths_ours)
        output_directory = (args.image_path).replace("/data/","/pred_ours_nocolor/")
    else:
    ...
    with torch.no_grad():
        for idx, image_path in enumerate(paths):
            ...
            disp_resized_np_scaled = ((disp_resized_np - disp_resized_np.min()) * 255 /   
                          (disp_resized_np.max() - disp_resized_np.min())).astype(np.uint8)  
            im = pil.fromarray(disp_resized_np_scaled, 'L')
